...
<p align="center">
  ...
  <a href="https://github.com/gregorygaines/bitfields-rs/blob/main/LICENSE"><img src="https://img.shields.io/github/license/Naereen/StrapDown.js.svg" alt="License"></a>
...
A Rust crate that provides a procedural macro for generating bitfields from
structs and enum bitflags for defining packed binary schemas in low-level
systems (e.g. embedded development or emulators).

### 🛠️ Core Features

- **Custom & Nested Bitfield Types**: Supports user-defined types, nested
  bitfields, and enum-based bitflags.
- **Bitflags**: Easily define bitflags using enums.
- **Array Support**: Supports array-backed bitfields and array fields, ideal for
  representing data exceeding 128 bits.
- **Endianness & Signed Data**: Built-in support for little/big endian
  conversions and 2's complement signed fields.
- **Flexible Fields**: Field defaults and ignored fields.

### ⚡ Performance

- **Safe & Lightweight**: No `unsafe`, zero allocations, no runtime
  dependencies, and constant memory usage for primitive bitfields.
- **`no_std` Compatible**: Works in embedded and standard library-free
  environments.
- **Efficient**: Generates efficient, `const`-friendly code comparable to
  handwritten implementations.

### 🛡️ Safety & Reliability

- **Compile-Time Validation**: Type, size, and bounds checks are validated at
  compile time.
- **Checked Accessors**: Optional checked accesses help catch out-of-bounds
  errors.
- **Well Tested**: Comprehensive testing with over 90% code coverage.

### 💻 Developer Experience

- **Generated Bit Operations**: Automatically generates operations such as
  `get`, `set`, and `invert`.
- **Flexible Code Generation**: Configurable generation for different schema
  layouts and use cases.
- **Editor-Friendly**: Preserves syntax highlighting on annotated structs and
  enums.
- **Documentation & Examples**: Comprehensive documentation that compiles and
  examples.
...
- [Usage](#-usage)
    ...
    - [Array Backed Bitfield](#array-backed-bitfield)
    - [Constructing a Bitfield](#constructing-a-bitfield)
        - [Bitfield Constructor](#bitfield-constructor)
        - [Bitfield Builder](#bitfield-builder)
    - [Bitfield Field Types](#bitfield-field-types)
        - [Signed Bitfield Fields](#signed-bitfield-fields)
        - [Array Bitfield Fields](#array-bitfield-fields)
        - [Checked Setters](#checked-setters)
        - [Field Access](#field-access)
        - [Custom Types](#custom-types)
        - [Nested Bitfields](#nested-bitfields)
        - [Reserved Fields](#reserved-fields)
    - [Bitflags](#bitflags)
        - [Bitflag Configuration](#bitflag-configuration)
    - [Field Constants](#field-constants)
    - [Field Order](#field-order)
    - [Bitfield Conversions](#bitfield-conversions)
        - [From Bits](#from-bits)
        - [Into Bits](#into-bits)
        - [Conversion Endian](#conversion-endian)
    - [Bit Operations](#bit-operations)
        - [Write Bits](#write-bits)
        - [Get/Set Bits](#getset-bits)
    ...
    - [Default Implementations](#default-implementations)
        ...
        - [From/Into Trait](#frominto-trait)
    - [Bitfield Internal Value](#bitfield-internal-value)
    - [Configuration](#configuration)
        - [Bit Operations Config](#bit-operations-config)
        - [Global Cargo Feature Flags](#global-cargo-feature-flags)
...
Imagine you are emulating the N1nt3nd0 and come across an 8-bit display register
defined as:

```text
  Bit   Expl.
  0-1   BG Mode                    (0=Picture, 1=Video, 2=Sleep, 3=Game)
  2-3   Color Data                 The color of the background, 1 by default.
  4     OBJ Character VRAM Mapping (0=Two dimensional, 1=One dimensional)
  5-7   Always 0x3                 Reserved, always 0x3
```

Laid out, the bits are as such:
...
<table>
  ...
  <tr>
    ...
    <td>OBJ</td>
    <td colspan="2">Color Data</td>
    <td colspan="2">BG Mode</td>
  </tr>
</table>

Using Bitfields, we can represent this register as follows:
...
#[bitfield(u8)]
#[derive(Hash)] // Attributes are passed to the struct.
struct DisplayControl {
    /// We specify bg mode occupies the first 2 bits (0-1) of the bitfield
    /// using the `#[bits]` attribute.
    #[bits(2)]
    bg_mode: BgMode,

    /// The color data occupies the next 2 bits (2-3) of the bitfield.
    /// We can also specify a default value for the field, which is 1
    /// in this case. The type of the field determines the maximum
    /// number of bits a field can occupy. This compiles because the
    /// 2 bits of this field can fit in a `u8` type.
    #[bits(2, default = 1)]
    color_data: u8,

    /// We can omit the `#[bits]` attribute for primitive types, the macro
    /// will assume the number of bits is the size of the field type. Here,
    /// it's 1 bit for a `bool` type.
    obj_char_vram_mapping: bool,

    /// Prefixing a field with "_" makes it a reserved field which
    /// is inaccessible. Reserved fields are 0 by default, unless a
    /// default value is provided.
    #[bits(3, default = 0x3)]
    _always_0x3_reserved: u8,
}

/// Define an enum annotated with the `#[bitflag(u8)]` attribute
/// that represents bitflags. One variant must be annotated with
/// `#[base]` which represents the base value of the bitflag, in
/// this case, `Picture`.
#[bitflag(u8)]
enum BgMode {
    #[base]
...
fn main() {
    // The bitfield can be constructed using a constructor.
    let mut display_control = DisplayControl::new();
    display_control.set_bg_mode(BgMode::Game);
    display_control.set_color_data(2);
    display_control.set_obj_char_vram_mapping(true);

    // The bitfield can be constructed using a builder pattern.
    let display_control_builder = DisplayControlBuilder::new()
        .with_bg_mode(BgMode::Game)
        .with_color_data(2)
        .with_obj_char_vram_mapping(true)
        .build();

    // Converting into bits.
    let display_builder_bits = display_control_builder.into_bits();
    assert_eq!(display_builder_bits, 0b01111011);

    let display_bits = display_control.into_bits();
    assert_eq!(display_bits, 0b01111011);
}
```

## 💡 Usage

Bitfields offers a wide range of features to help you define and work with
bitfields and flags.
...
<a href="https://www.star-history.com/?repos=gregorygaines%2Fbitfields-rs&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=gregorygaines/bitfields-rs&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=gregorygaines/bitfields-rs&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=gregorygaines/bitfields-rs&type=date&legend=top-left" />
 </picture>
</a>
...
- [Johan Mickos](https://github.com/johanmickos) - Awesome dude who gave me
  the 'bitfields' crate name.
